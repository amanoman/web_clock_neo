...
describe('ClockDisplay', () => {
  ...
  it('日付要素が時刻要素より前（DOM順）', () => {
    const { container } = render(<ClockDisplay {...defaultProps} />)
    const times = container.querySelectorAll('time')
    expect(times).toHaveLength(2)
    expect(times[0].textContent).toBe('2026年4月15日 水曜日')
    expect(times[1].textContent).toBe('09:05:03')
  })

  it('dateTime 属性が正しく設定されている', () => {
    const { container } = render(<ClockDisplay {...defaultProps} />)
    const times = container.querySelectorAll('time')
    expect(times[0]).toHaveAttribute('dateTime', '2026-04-15')
    expect(times[1]).toHaveAttribute('dateTime', '09:05:03')
  })
})
